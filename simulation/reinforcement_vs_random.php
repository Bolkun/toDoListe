<style>
	#reinerForcementButton {
		float: right;
	}
	#reinerForcementButtonOut {
		float: right;
	}
</style>
<div id='reinerforcement_php'>
	<div style='text-align: right;'>
		<!--<button class="btn btn-info b_location" id="reinerForcementButton" type="button" onclick="rf_training_aktivate()">Reiner Forcement</button>-->
	</div>
	<div id='rf_training'>
		<button class="btn btn-info b_location" id="reinerForcementButtonOut" type="button" onclick="rf_training_deaktivate()">End Simulation</button>
		<!--mysql-->
		<?php
		...
		<?php
			//DELETE
			mysql_query("DELETE FROM score WHERE Agent1='Reinforcement' AND Agent2='Random'");
			mysql_query("DELETE FROM memory");
			echo "<div id='battleProtokol' style='margin-left: 33.33%; margin-top: 50px;'>";
				...
				$rf_points2 = 0;
				for($i=0; $i<$play; $i++){
					//agent1 (Reinforcement Learning "TD(0)") start card
					$a1_m_start = rand(0, 5);	//0,5
					echo "Agent1: Choose: $a1_m_name[$a1_m_start]($a1_m_start) ($a1_m_c_hp[$a1_m_start])<br>";
					//agent2 (Random) start card
					$a2_m_start = rand(0, 5);
					echo "Agent2: Choose: $a2_m_name[$a2_m_start]($a2_m_start) ($a2_m_c_hp[$a2_m_start])<br>";
					//win cases
					...
					while(($a1_sum_c_hp != 0) || ($a2_sum_c_hp != 0)){
						//check memory of agent1
						$query = "SELECT State1, State2, Action, Reward FROM memory";	
						$result = mysql_query($query) or die("Query failed : " . mysql_error());
						while ($aRow = mysql_fetch_array($result)) {
							//Agent1
							$m_state1[] = $aRow["State1"];
							$m_state2[] = $aRow["State2"];
							$m_action[] = $aRow["Action"];
							$m_reward[] = $aRow["Reward"];
						}
						if (mysql_num_rows($result)==0){	
							$m_anz_memory = 0;
						} else {
							$m_anz_memory = count($m_state1);
						}
						mysql_free_result($result);
						...
						if($m_anz_memory != 0){
							for($c=0; $c<$m_anz_memory; $c++){
								//check if action exist in memory
								if(($m_state1[$c]==$a1_m_start) && ($m_state2[$c]==$a2_m_start) && ($m_action[$c]==$a1_action)){	
									$in_memory = true;
									break;
								}
							}
							if($in_memory == true){
								$m_reward_check = 0;
								//echo "in memory<br>";
								//find action with best reward
								for($r=0; $r<$m_anz_memory; $r++){
									if(($m_state1[$r]==$a1_m_start) && ($m_state2[$r]==$a2_m_start) && ($a1_action != $m_action[$r])){	//
										//availible action ?
										for($h=0; $h<$a1_anz_action_available; $h++){
											if($m_action[$r] == $a1_action_available[$h]){
												$in_array = true;
												break;
											}
										}
										if($in_array == true){
											if($m_reward_check < $m_reward[$r]){
												$m_reward_check = $m_reward[$r];
												$a1_action = $m_action[$r];
												//echo "Reward: $m_reward_check Aktion: $a1_action<br>";
											}
										}
										$in_array = false;
									}
								}
								//echo "New Action: $a1_action<br>";
							}
						}
						...
						if($a1_action == 4){
							...
							//check effectivity
							for($s=0; $s<$rf_anz_types_effectivity; $s++){
								if(($rf_te_type1[$s] == $a1_m_type1[$a1_m_start]) && ($rf_te_type2[$s] == $a2_m_type1[$a2_m_start])){
									$c_effectivity = $rf_te_effectivity[$s];
									break;
								}
							}
							/************************************************************************
							Agent1 knows no effectivity
							***********************************************************************/
							//change card
							$a1_change_decision = rand(0, $a1_anz_cards_available_to_change - 1);
							$a1_m_start = $a1_cards_available_to_change[$a1_change_decision];
							echo "Agent1: Repick: $a1_m_name[$a1_m_start]($a1_m_start)<br>";
							...
							if($c_effectivity == 2){
								$a2_m_status = 'changed';
								$a2_m_damage = '50';
							}else if($c_effectivity == 1){
								$a2_m_status = 'changed';
								$a2_m_damage = '100';
							}else if($c_effectivity == 0.5){
								$a2_m_status = 'changed';
								$a2_m_damage = '150';
							}else if($c_effectivity == 0){
								$a2_m_status = 'changed';
								$a2_m_damage = '200';
							}
							/**********************************************************************/
							
							/*if(($c_effectivity == 2) && ($a1_anz_action_available > 1)){
								//dont change delete option 4
								for($a=0; $a<$a1_anz_action_available; $a++){
									if($a1_action_available[$a] == 4){
										$a1_action_available[$a1_anz_action_available-1];
									}
								}
								$a1_anz_action_available = $a1_anz_action_available - 1;
								$a1_action_decision = rand(0, $a1_anz_action_available - 1);	//0, 1, 2, 3
								$a1_action = $a1_action_available[$a1_action_decision];
							...
							if($c_effectivity == 1){
								//find if exist effectivity 2
								$a1_old_m_start = $a1_m_start;
								for($s=0; $s<$a1_anz_cards_available_to_change; $s++){
									$mon_counter = $a1_cards_available_to_change[$s];
									//check effectivity
									for($d=0; $d<$rf_anz_types_effectivity; $d++){
										if(($a1_m_type1[$mon_counter] == $rf_te_type1[$d]) && ($rf_te_type2[$d] == $a1_m_type1[$a2_m_start]) && ($rf_te_effectivity[$s] == 2)){
											$a1_m_start = $a1_cards_available_to_change[$s];
											break;
										}
									}
								}
								if($a1_old_m_start == $a1_m_start){
									$in_memory = false;
									$in_array = false;
							...
							if($c_effectivity == 0.5){
								$a1_old_m_start = $a1_m_start;
								//find if exist effectivity 1
								for($s=0; $s<$a1_anz_cards_available_to_change; $s++){
									$mon_counter = $a1_cards_available_to_change[$s];
									//check effectivity
									for($d=0; $d<$rf_anz_types_effectivity; $d++){
										if(($a1_m_type1[$mon_counter] == $rf_te_type1[$d]) && ($rf_te_type2[$d] == $a1_m_type1[$a2_m_start]) && ($rf_te_effectivity[$s] == 1)){
											$a1_m_start = $a1_cards_available_to_change[$s];
											break;
										}
									}
							...
							if($c_effectivity == 0){
								$a1_old_m_start = $a1_m_start;
								//find if exist effectivity 0.5
								for($s=0; $s<$a1_anz_cards_available_to_change; $s++){
									$mon_counter = $a1_cards_available_to_change[$s];
									//check effectivity
									for($d=0; $d<$rf_anz_types_effectivity; $d++){
										if(($a1_m_type1[$mon_counter] == $rf_te_type1[$d]) && ($rf_te_type2[$d] == $a1_m_type1[$a2_m_start]) && ($rf_te_effectivity[$s] == 0.5)){
											$a1_m_start = $a1_cards_available_to_change[$s];
											break;
										}
									}
						...
						if($a2_action == 4){
							...
							echo "Agent2: Repick: $a2_m_name[$a2_m_start]($a2_m_start)<br>";
						}
						//compare speed
						...
						//unset selected arrays
						unset($m_state1);
						unset($m_state2);
						unset($m_action);
						unset($m_reward);
						//mysql insert new action
						if($in_memory == false){
							//if(($a1_action == 0) || ($a1_action == 1) || ($a1_action == 2) || ($a1_action == 3)){
								if($a2_m_status != 'missed'){
									$sql_insert = "INSERT INTO memory VALUES ('', $a1_m_start, $a2_m_start, $a1_action, $a2_m_damage);";
									$result_insert = mysql_query($sql_insert) or die(mysql_error());
									// Select duplicate entries
									$result = mysql_query("SELECT ID FROM memory GROUP BY State1, State2, Action HAVING COUNT(*) > 1");
									// Loop through the results
									while($aRow = mysql_fetch_array($result)){
										$multiple_memory_id[] = $aRow['ID'];
									}
									if (mysql_num_rows($result)==0){	
										$anz_multiple_memory_id = 0;
									} else {
										$anz_multiple_memory_id = count($multiple_memory_id);
									}
									
									// Delete each row by id
									if($anz_multiple_memory_id != 0){
										for($d=0; $d<$anz_multiple_memory_id; $d++){
											mysql_query("DELETE FROM memory WHERE ID ='$multiple_memory_id[$d]'");
										}
									}
									mysql_free_result($result);
								}
					...
					$current_match = $i + 1;
					$sql_insert = "INSERT INTO score VALUES ('', '$rf_datetime', 'Reinforcement', 'Random', '$current_match', '$rf_points1', '$rf_points2');";
					$result_insert = mysql_query($sql_insert) or die(mysql_error());
				...
				//show score
				$query = "SELECT Match_Played, Points1, Points2 FROM score WHERE Agent1='Reinforcement' AND Agent2='Random'";	
				$result = mysql_query($query) or die("Query failed : " . mysql_error());
				while ($aRow = mysql_fetch_array($result)) {
					//Agent1
				...
				$a1_proz_score= $a1_points1[0] * 100 / $match_played[0];
				$a2_proz_score= $a2_points2[0] * 100 / $match_played[0];
				echo "<br><p style='font-weight: cursor;'>In $match_played[0] matches<br> Agent1 score $a1_points1[0]($a1_proz_score%) points<br> Agent2 score $a2_points2[0]($a2_proz_score%) points</p>";
				mysql_free_result($result);
			echo "</div>";
		?>
	</div>
